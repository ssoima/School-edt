...
JNIEXPORT jint JNICALL Java_GRNVS_1RAW_checksum_1
  ...
  jbyteArray payload, jint payload_offset, jint payload_length)
{
	...
	(*env)->GetByteArrayRegion(env, payload, payload_offset, payload_length,
								payloadbuffer);
	return icmp6_checksum((struct ip6_hdr*)hdrbuffer, payloadbuffer,
								payload_length);
}


JNIEXPORT void JNICALL Java_GRNVS_1RAW_print_1
